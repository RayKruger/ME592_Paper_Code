...
def setup_and_run_train(n_channels, n_classes, dir_img, dir_gt, dir_results, load, 
    ...
    for epoch in range(epochs):
        ...
        train_loss = train_net(net, device, train_loader, optimizer, criterion, batch_size,epoch+1, isWCE = (loss == "WCE"))
        train_loss_history.append(train_loss)
        val_loss = val_net(net, device, val_loader, criterion, batch_size,epoch+1)
    ...
    save_dir = 'plots'
    plt.savefig(os.path.join(save_dir, 'loss_plots_' + run + '.png'))
    plt.close(fig)
