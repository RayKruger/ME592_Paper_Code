...
def see_results(n_channels, n_classes, load_weights, dir_img, dir_cmp, savedir, title):
    ...
    with torch.no_grad():
        for batch_idx, (data, gt, weights) in enumerate(loader):
            ...
            save_image(predictions, savedir + title + "_pred_{}.png".format(i))
            save_image(gt, savedir + title + "_gt_{}.png".format(i))
    epoch = np.arange(1, len(accuracy_history) + 1)
    plt.plot(epoch, accuracy_history)
    plt.xlabel('image')
    plt.ylabel('Test Accuracy')
    plt.title('Test Accuracy Curve')
    plt.savefig(savedir + '/test_accuracy_plot_epoch{}.png'.format(epoch))
    plt.close()
...
if __name__ == '__main__':
    ...
    see_results(n_channels = args.n_channels, 
        ...
        dir_img = "img_test/pre_0_3/", 
        dir_cmp = "img_test/one_class/", 
        savedir = args.folder, 
        title = args.title)
